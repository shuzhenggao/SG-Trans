...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    assert len(sources) == len(source_sub) == len(source_tags) == len(targets)==len(source_ky) == len(source_it) == len(source_is) ==\
           source_dataflow_sp.shape[0] == source_controlflow_sp.shape[0]

    examples = []
    for src, src_sub, src_tag, tgt, src_ky, src_it, src_is, src_df_sp, src_cf_sp in\
        tqdm(zip(sources, source_sub, source_tags, targets, source_ky, source_it, source_is, source_dataflow_sp, source_controlflow_sp),
                                  total=len(sources)):
