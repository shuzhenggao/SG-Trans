...
def vectorize(ex, model):
    ...
    vectorized_ex['code_type5'] = list(map(int,code.type5))
...
def batchify(batch):
    ...
    code_type5 = [ex['code_type5']+['0' for _ in range(max_code_len-len(ex['code_type5']))] for ex in batch]
    ...
    code_type5_rep = torch.Tensor(list(list(map(int,i)) for i in code_type5))
    code_intoken_map = get_local_map(code_intoken)
    code_instatement_map = get_local_map(code_instatement)
