...
# from c2nl.modules.type_ffn import PositionwiseFeedForward
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, inputs, mask,code_keyword,code_type5,code_intoken,code_instatement,code_dataflow,heads_type):
        """
        ...
        context, attn_per_head, _ = self.attention(inputs, inputs, inputs,code_keyword, code_intoken, code_instatement,
                                                   code_dataflow, heads_type, mask=mask, attn_type="self")
        #print(context.shape)
        #print(inputs.shape)
        out = self.layer_norm(self.dropout(context) + inputs)
        #input('ok:')
        return self.feed_forward(out,code_type5), attn_per_head
...
class TransformerEncoder(EncoderBase):
    ...
    def forward(self, src,code_keyword,code_type5,code_intoken,code_instatement,code_dataflow,lengths=None):
        """
        Args:
        ...
        for i in range(self.num_layers):
            out, attn_per_head = self.layer[i](out, mask, code_keyword, code_type5, code_intoken, code_instatement,
                                               code_dataflow, layers_heads_type[i])
